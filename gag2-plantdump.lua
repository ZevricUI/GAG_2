--[[
  ...
  Prints everything about the first few plants on your plot: name,
  every attribute, children and their attributes. Whatever holds the
  crop name is in here.

  Run it while standing on your plot, then paste the result back.
--]]
...
--========================================================
-- Find the plot the same way the script does.
--========================================================
local plot = nil
	...
	if container:IsA("Folder") or container:IsA("Model") then
		...
		if ok then
			for _, candidate in ipairs(children) do
				if candidate:GetAttribute("Owner") == player.Name then
					plot = candidate
					break
				end
			end
...
if not plot then
	add("No plot found with Owner = " .. player.Name, "bad")
else
	add("Plot: " .. plot:GetFullName(), "good")
	local plants = plot:FindFirstChild("Plants")
	if not plants then
		add("No 'Plants' folder inside the plot.", "bad")
		add("Plot children:")
		for _, child in ipairs(plot:GetChildren()) do
			add("  " .. child.Name .. " [" .. child.ClassName .. "]")
		end
	else
		local all = plants:GetChildren()
		add("Plants folder holds " .. #all .. " plant(s). Showing up to 3:", "good")

		for index = 1, math.min(3, #all) do
			local plant = all[index]
			add("")
			add("=== PLANT " .. index .. " ===", "good")
			add("  Name: " .. plant.Name)
			add("  ClassName: " .. plant.ClassName)
			...
			for _, child in ipairs(plant:GetChildren()) do
				add("    " .. child.Name .. " [" .. child.ClassName .. "]")
				local okc, cattrs = pcall(function() return child:GetAttributes() end)
				if okc and cattrs then
					for key, value in pairs(cattrs) do
						add("       attr " .. tostring(key) .. " = " .. tostring(value))
					end
				end
			end
		end
...
-- Clipboard + UI
--========================================================
local dumpParts = { "Zev Hub — Plant dump", "" }
